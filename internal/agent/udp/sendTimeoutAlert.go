...
func sendTimeoutAlertAndEmptyMetrics(command string, taskID uint16, agentID byte, startTime time.Time) {
	...
	newMetrics := metrics.NewMetricsBuilder().
		SetPacketID(metricsID).
		SetAgentID(agentID).
		SetTaskID(taskID).
		SetTime(startTime.Format("15:04:05.000000000")).
		SetCommand(command).
	...
	ack.SendPacketAndWaitForAck(metricsID, agentID, packetsWaitingAck, &pMutex, serverConn, nil, packetData, "[AGENT] [MAIN READ THREAD] Empty metrics packet sent", "[AGENT] [ERROR 331] Unable to send empty metrics packet")
}
