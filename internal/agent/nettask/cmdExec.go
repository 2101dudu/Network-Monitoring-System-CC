...
func handleCpuUsage(conditions task.AlertFlowConditions, taskID uint16) bool {
	...
	if cpuUsage > float32(conditions.CpuUsage) {
		...
		buildAlert := alert.NewAlertBuilder().SetPacketID(newPacketID).SetAgentID(agentID).SetTaskID(taskID).SetAlertType(alert.CPU).SetExceeded(cpuUsage).SetTime(alertTime.Format("15:04:05.000000000"))
...
func handleRamUsage(conditions task.AlertFlowConditions, taskID uint16) bool {
	...
	if ramUsage > float32(conditions.RamUsage) {
		...
		newPacketID := utils.ReadAndIncrementPacketID(&packetID, &packetMutex, true)
		buildAlert := alert.NewAlertBuilder().SetPacketID(newPacketID).SetAgentID(agentID).SetTaskID(taskID).SetAlertType(alert.RAM).SetExceeded(ramUsage).SetTime(alertTime.Format("15:04:05.000000000"))
...
func handleInterfaceStats(interfaceName string, conditions task.AlertFlowConditions, taskID uint16) bool {
	...
	if float32(interfaceStats) > float32(conditions.InterfaceStats/4) {
		...
		newPacketID := utils.ReadAndIncrementPacketID(&packetID, &packetMutex, true)
		buildAlert := alert.NewAlertBuilder().SetPacketID(newPacketID).SetAgentID(agentID).SetTaskID(taskID).SetAlertType(alert.INTERFACESTATS).SetExceeded(float32(interfaceStats * 4)).SetTime(alertTime.Format("15:04:05.000000000"))
