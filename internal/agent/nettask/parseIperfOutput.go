...
func parseIperfOutput(bandwidth bool, jitter bool, packetLoss bool, jitterLimit float32, packetLossLimit float32, output string) (string, float32, float32) {
	...
	if bandwidth {
		...
		if jitter {
			...
			if err != nil {
				log.Println(utils.Red, "[ERROR 155] Transforming jitter string into float", utils.Reset)
			}
		...
		if packetLoss {
			...
			if err != nil {
				log.Println(utils.Red, "[ERROR 156] Transforming packet loss percentage string into float", utils.Reset)
			}
